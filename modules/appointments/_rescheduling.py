...
@tool
def reschedule_appointment(context:ToolContext, doctor_name:str, scheduled_date:datetime, requested_date:datetime) -> ToolResult:
...
def _verify_update(patient_id:str, doctor_name:str, scheduled_date:datetime, requested_date:datetime) -> dict[Any, str]:
    ...
    if doctor_availabilities is None:
        return {"error": "Issue with the appointment rescheduling."}
    ...
    doctor_patient_upcoming_match = ((entry for entry in doctor_patient_upcoming if entry["date"] == requested_date_slot and entry["time"] == requested_time_slot),None)
    doctor_patient_upcoming_non_match = ((entry for entry in doctor_patient_upcoming if entry["date"] == scheduled_date_slot and entry["time"] == scheduled_time_slot),None)
    
    if doctor_patient_upcoming_match is None or doctor_patient_upcoming_non_match:
        return {"error": "Issue with the appointment rescheduling."}
